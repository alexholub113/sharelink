...
const TagBadge = ({name, count, onClick, active, removable, children}: TagLabelProps) => (
    <Button onClick={onClick}
            className={`py-1
                     ${active ? 'card-background rounded-full px-3': ''}`}>
        <span>{formatTagName(name)}</span>
        { !active && count && <span className="ml-1 text-xs font-normal text-gray-500 dark:text-zinc-400 dark:bg-zinc-700 rounded-full px-1">{count}</span>}
