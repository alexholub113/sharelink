...
const AddTagButton = ({ onAdd }: TagInputProps) => {
    ...
    return (
        <>
            {isInputVisible ? (
                <TagSearchInput onTagClick={handleOnAdd} onSuggestionListClose={() => setInputVisible(false)} showSuggestionsInitial />
            ) : (
                <Button onClick={() => setInputVisible(true)} className="px-3 flex font-bold flex-row gap-2 secondary-text-color
                rounded-full cursor-pointer border bg-zinc-900 border-zinc-800 p-2 hover:scale-105">
