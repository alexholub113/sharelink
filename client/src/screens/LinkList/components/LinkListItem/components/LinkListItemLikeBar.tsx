...
type LinkListItemLikeBarProps = Pick<Link, 'isLiked' | 'likes' | 'isSaved' | 'id'>;

const LinkListItemLikeBar = observer(({ isLiked, likes, isSaved, id }: LinkListItemLikeBarProps) => {
    ...
    return (
        <div className="flex flex-row justify-between secondary-text-color">
            <button className="flex bg-transparent items-center justify-center text-center" onClick={() => likeLink(id)}>
                ...
                { likes > 0 && <span className="ml-2 pt-2 secondary-text-color text-sm">{shortLikes(likes)}</span> }
            </button>
