...
const UserInteractionsButton = ({onClick, active, children, groupSide, disabled}: UserInteractionButtonProps) => (
    <Button onClick={onClick} disabled={disabled} type="button" disableAnimation
...
const UserInteractionsFilter = observer(() => {
    const { toggleLikedFilter, toggleSavedFilter, toggleOwnedFilter, state: { filter: { liked, saved, owned }, isListLoading}} = useLinkStore();
    return (
        <div className="inline-flex rounded-full shadow-sm" role="group">
            <UserInteractionsButton onClick={() => toggleLikedFilter()} active={liked} groupSide="left" disabled={isListLoading}>
            ...
            <UserInteractionsButton onClick={() => toggleSavedFilter()} active={saved} groupSide="middle" disabled={isListLoading}>
            ...
            <UserInteractionsButton onClick={() => toggleOwnedFilter()} active={owned} groupSide="right" disabled={isListLoading}>
